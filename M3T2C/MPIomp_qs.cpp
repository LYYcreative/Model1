...
//quick sort
void qsort(int left, int right, int arr[],int numtasks)
{
    if(left>right){
        return;
    }
    ...
    arr[left] = arr[i];
    arr[i] = base;
    qsort(left, i-1, arr,threads);
    qsort(i+1, right, arr,threads);
}
...
int main(int argc, char** argv)
{
    ...
    qsort(0,length-1,sub_v1,numtasks);//sort each sub_v1
